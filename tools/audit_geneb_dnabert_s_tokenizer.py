...
"""Audit DNABERT-S's overridden backend length state and native BPE parity."""
...
TOKENIZER_SHA256 = "667f5a0e6e9630f7052b004784dda7c7aeba395cc4ca3c01b11b02e6aa29d7b1"
TOKENIZER_SIZE = 168203
INPUTS = (
    ("canonical", "ACGTNACGTNACGTN"),
    ("bpe-and-unknown", "AAAACCCCGGGGTTTTACGTNACGTNNNNACGTTGCAACGTACGT"),
    ("hf-whitespace", "A C\tG\nT"),
    ("case-preserved", "acgt"),
    ("unknown-n", "N"),
    ("empty", ""),
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("pinned DNABERT-S tokenizer source differs")
    manifest = read_json(manifest_path, "compiler manifest")
    options = manifest.get("options")
    if not isinstance(options, dict) or options.get(
        "ignore_hf_backend_truncation_padding"
    ) is not True:
        raise RuntimeError("DNABERT-S manifest omitted the B80 explicit override audit")
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        str(snapshot), use_fast=True, local_files_only=True, trust_remote_code=True
    )
    ...
    if tokenizer.backend_tokenizer.truncation != expected_backend_truncation:
        raise RuntimeError("source backend truncation state differs")
    if tokenizer.backend_tokenizer.padding != expected_backend_padding:
        raise RuntimeError("source backend padding state differs")
    direct_backend = tokenizer.backend_tokenizer.encode(INPUTS[0][1])
    if len(direct_backend.ids) != 2000:
        raise RuntimeError("source backend fixed-padding state was not observable")
    ...
    overridden_backend_truncation = {
        "max_length": 512,
        "stride": 0,
        "strategy": "longest_first",
        "direction": "right",
    }
    overridden_backend_padding = {
        "length": None,
        "pad_to_multiple_of": None,
        "pad_id": 3,
        "pad_token": "[PAD]",
    ...
    if tokenizer.backend_tokenizer.truncation != overridden_backend_truncation:
        raise RuntimeError("extractor-call truncation override was not observable")
    if tokenizer.backend_tokenizer.padding != overridden_backend_padding:
        raise RuntimeError("extractor-call padding override was not observable")
    generator_sha256 = sha256_file(Path(__file__).resolve(strict=True))
    report = {
        "schema_version": 1,
        "kind": "geneb-dnabert-s-tokenizer-parity-audit",
        "status": "passed",
        "source": {
        ...
        "b80": {
            "source_backend_truncation": expected_backend_truncation,
            "source_backend_padding": expected_backend_padding,
            "direct_backend_canonical_token_count": len(direct_backend.ids),
            "extractor_call_override": {
                "padding": True,
                "truncation": True,
                "max_length": 512,
                "batch_size": 1,
            },
            "backend_truncation_after_extractor_call": overridden_backend_truncation,
            "backend_padding_after_extractor_call": overridden_backend_padding,
            "manifest_opt_in": "ignore_hf_backend_truncation_padding=true",
        },
        "b77": {
