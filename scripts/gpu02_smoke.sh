...
test -x "$build_dir/evo2c-cuda-pipeline-tests"
...
for binary in "$build_dir/evo2c" "$build_dir/evo2c-inspect" \
              "$build_dir/evo2c-cuda-pipeline-tests"; do
...
apptainer exec --nv -B "$nix_root:/nix:ro" "$image" cuobjdump \
  --list-elf "$build_dir/evo2c-cuda-pipeline-tests" |
