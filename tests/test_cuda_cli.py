...
def main() -> int:
    ...
    run_alias = run_checked([str(args.binary), "run", *generation_command[1:]])
    if run_alias.stdout != first.stdout:
        raise AssertionError("run subcommand changed legacy CUDA generated bytes")
    ...
    if score_alias.stdout != score_result.stdout:
        raise AssertionError("score subcommand changed legacy CUDA scoring JSONL")
    benchmark_result = run_checked(
        [
            str(args.binary),
            "bench",
            "-m",
    ...
    if any(
        (
            benchmark["architecture"] != "StripedHyena2Test",
            benchmark["artifact_profile"] != "evo2-runtime-v1",
            benchmark["execution_profile"] != "exact",
            benchmark["backend"] != "cuda",
            benchmark["tokens"] != 4,
            benchmark["repetitions"] != 2,
            len(benchmark["samples_seconds"]) != 2,
            benchmark["median_tokens_per_second"] <= 0,
        )
    ):
        raise AssertionError("CUDA benchmark omitted reproducibility identity")
