...
"""Compare GENEB long-Hyena native fixtures with an independent oracle."""
...
def compare_vectors(
    ...
    for name in sorted(expected_vectors):
        ...
        if maximum > tolerance:
            raise AssertionError(
                "%s max_abs %.9g exceeds %.9g" % (name, maximum, tolerance)
            )
    if expected.get("work") != actual.get("work"):
        raise AssertionError("native/oracle complexity counters differ")
...
def main() -> int:
    ...
    compare_vectors(expected, generated, 0.0)
    native = run_json([str(args.binary), "--dump-vectors"])
    compare_vectors(expected, native, 1.0e-5)
    print("GENEB long-Hyena NumPy oracle vectors passed")
    return 0
