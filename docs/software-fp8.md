...
2. Reproduce Vortex's parameter-loading dtype before quantizing the static
   weight to one-byte E4M3FN codes. The 1B/40B sources are already BF16. The
   20B source is F32, but Vortex copies it into a BF16 model parameter before
   Transformer Engine sees it, so its exact path is F32 → BF16 → E4M3.
...
- The converter verifies the delayed-scale metadata and reproduces the exact
  PyTorch load-time BF16 boundary. On 20B layer 0, omitting that boundary
  changed 2,619,436 E4M3 codes and failed at the first projection.
- Independent Vortex/PyTorch and native executions of 1B, 20B, and 40B match
  every audited prefill block, cached-decode block, logit, and generated byte
  at raw-bit level; see the [model-size record](model-size-validation.md).
