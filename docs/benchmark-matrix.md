...
The corresponding three-size ESMC comparison is
`scripts/gpu02_benchmark_esmc.sh`. It uses the shared high-capacity
`HF_HOME`, refuses a busy GPU, pins and hashes the official source and model
revisions, times the same F32 batch-one host-logits scope, and retains every
sample plus a recursively hashed report.
