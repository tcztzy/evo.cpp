...
The whole-file NPY statements above describe that archived binary and writer.
The current libnpy writer may use different standards-compliant header padding;
the maintained compatibility contract is exact shape, row-major dtype, and raw
array element bits. Readers locate the payload from the declared header length,
so both container layouts remain readable. Generated sequence files are still
...
0`. The CLI regression for the archived run compared complete NPY files
byte-for-byte. Current cross-writer checks compare parsed raw array bits, with
